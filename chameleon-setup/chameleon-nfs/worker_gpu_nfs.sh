...
apt-get install -y build-essential make zlib1g-dev librrd-dev libpcap-dev autoconf libarchive-dev iperf3 htop bmon vim wget pkg-config git python-dev python-pip libtool nfs-common
...
cat << EOF >> /etc/hosts
192.168.100.128 panorama-data-nfs
EOF
...
EXECUTE = /nfs/shared/condor/execute

CONDOR_HOST = panorama-data-nfs
...
echo 'export PATH=/usr/local/cuda-10.2/bin${PATH:+:${PATH}}' >> /home/panorama/.bashrc
echo 'export LD_LIBRARY_PATH=/usr/local/cuda-10.2/lib64${LD_LIBRARY_PATH:+:${LD_LIBRARY_PATH}}' >> /home/panorama/.bashrc

echo 'export GOPATH=${HOME}/go' >> /home/panorama/.bashrc
echo 'export PATH=/usr/local/go/bin:${PATH}:${GOPATH}/bin' >> /home/panorama/.bashrc

mkdir -p /nfs/shared
echo "panorama-data-nfs:/nfs/shared  /nfs/shared   nfs      defaults  0     0" >> /etc/fstab
mount -a

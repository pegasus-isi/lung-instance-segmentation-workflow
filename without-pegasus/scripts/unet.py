...
class UNet:
    ...
    def DataLoader(self):
        ...
        """
        path = os.path.join(self.curr, "data")   
        train_masks = []
        val_masks = []
        all_files = []

        train_path = os.path.join(path, "train_processed")
        test_path = os.path.join(path, "test_processed")
        val_path = os.path.join(path, "val_processed")
        mask_path = os.path.join(path, "lung-masks")
        ...
        y_train = [cv2.resize(cv2.imread(os.path.join(mask_path,i)),(dim, dim))[:,:,0] for i in train_masks]

        X_valid = [cv2.imread(os.path.join(val_path,i))[:,:,0] for i in val_data]
        y_valid = [cv2.resize(cv2.imread(os.path.join(mask_path,i)), (dim, dim))[:,:,0] for i in val_masks]

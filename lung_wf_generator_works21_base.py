...
def run_workflow(args):
    ...
    unet_wf_cont = Container("unet_wf",
            ...
            image="http://panorama-data/~panorama/containers/lung-segmentation_latest.sif",
            image_site="local"
    )
    ...
    hpo_task = Transformation( 
                    "hpo",
                    site="local",
                    pfn="http://panorama-data/~panorama/bin/LungSegmentation/hpo.py",
                    is_stageable=True,
                    container=unet_wf_cont
                )\
    ...
    train_model = Transformation( 
                    "train_model",
                    site="local",
                    pfn="http://panorama-data/~panorama/bin/LungSegmentation/train_model.py",
                    is_stageable=True,
                    container=unet_wf_cont
                )\
                .add_pegasus_profiles(label="cluster-1", cores=24, gpus=1, memory=131072, runtime=43200, grid_start_arguments="-G -m 10")\
    ...
    predict_masks = Transformation( 
                    "predict_masks",
                    site="local",
                    pfn="http://panorama-data/~panorama/bin/LungSegmentation/prediction.py",
                    is_stageable=True,
                    container=unet_wf_cont
                )\
                .add_pegasus_profiles(label="cluster-1", cores=24, gpus=1, memory=131072, runtime=43200, grid_start_arguments="-G -m 10")\

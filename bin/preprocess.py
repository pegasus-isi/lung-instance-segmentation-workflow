...
if __name__=="__main__":
    ...
    for ind, i in enumerate(all_files):
        if ".png" in i: 
            if args.cutoff=='t':
                if ind > upper_limit: break
                if ind < lower_limit: continue
            if "mask" in i: all_masks.append(i)
            else: images.append(i)
    if args.cutoff=='t': all_masks = os.listdir(args.mask_dir)
        ...
        if args.type == "train": 
            ...
            for m in all_masks:
                ...
                if fname == mname:
                    mask_img = m
                    break
            if mask_img is None: print('Not found ', i, fname); continue

...
def train_test_val_split(preprocess, training_input_files, mask_files, processed_training_files, processed_val_files, processed_test_files, training_masks, val_masks, test_masks, num_inputs):
    np.random.seed(4)
    process_jobs = [Job(preprocess).add_args("preprocess.py", "--type", group).add_inputs(File("preprocess.py")) for group in ["train", "val", "test"]]
    augmented_masks = []
    ...
    for fname in ["bin/unet.py", "bin/utils.py", "bin/preprocess.py", "bin/hpo.py", "bin/train_model.py", "bin/prediction.py", "bin/evaluate.py"]:
...
def run_workflow(args):
    ...
    preprocess = Transformation(
                    "preprocess",
                    site = "condorpool",
                    pfn="file:///nfs/shared/panorama/bin/LungSegmentation/darshan_wrapper.sh",
                    is_stageable=False,
    ...
    hpo_file = File("hpo.py")
    hpo_job = Job(hpo_task)\
                    .add_args(hpo_file)\
                    .add_inputs(hpo_file, *processed_training_files, *processed_val_files, *training_masks, *val_masks, unet_file)\
    ...
    train_model_file = File("train_model.py")
    train_job = Job(train_model)\
                    .add_args(train_model_file)\
                    .add_inputs(train_model_file, study_result, *processed_training_files, *processed_val_files, *training_masks, *val_masks, unet_file, utils_file)\
                    .add_outputs(model)
    ...
    prediction_file = File("prediction.py")
    predict_job = Job(predict_masks)\
                    .add_args(prediction_file)\
                    .add_inputs(prediction_file, model, *processed_test_files, unet_file)\
    ...
    evaluate_job = Job(evaluate_model)\
                    .add_args(evaluate_file)\
                    .add_inputs(evaluate_file, *processed_training_files, *processed_test_files, *predicted_masks, *test_masks, unet_file)\
